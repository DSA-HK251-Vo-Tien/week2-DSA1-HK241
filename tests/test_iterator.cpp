...
TEST_SUITE("DoublyLinkedList Iterator")
{
    ...
    TEST_CASE("Modifying values during iteration") {
        DoublyLinkedList<int> list;
        for (int i = 1; i <= 3; ++i) list.insertAtTail(i); // [1,2,3]

        for (auto &v : list) v *= 2; // [2,4,6]

        int expected[] = {2, 4, 6};
        int idx = 0;
        for (auto &v : list) CHECK(v == expected[idx++]);
    }

    TEST_CASE("Multiple iterator passes") {
        DoublyLinkedList<int> list;
        list.insertAtTail(1);
        list.insertAtTail(2);

        int sum1 = 0, sum2 = 0;
        for (auto v : list) sum1 += v;
        for (auto v : list) sum2 += v;

        CHECK(sum1 == sum2);
        CHECK(sum1 == 3);
    }
    TEST_CASE("Sum elements using iterator") {
        DoublyLinkedList<int> list;
